...
exports.SourcingTools = void 0;
const core_1 = require("@nitrostack/core");
const supabase_service_js_1 = require("../../services/supabase.service.js");
const SearchMaterialsSchema = core_1.z.object({
    material_type: core_1.z.string().optional().describe('Filter by material type'),
    min_quantity_kg: core_1.z.number().positive().optional().describe('Minimum quantity in kg'),
    max_price_per_kg: core_1.z.number().positive().optional().describe('Maximum price per kg'),
    buyer_lat: core_1.z.number().optional().describe('Buyer latitude for proximity sorting'),
    buyer_lng: core_1.z.number().optional().describe('Buyer longitude for proximity sorting'),
    max_radius_km: core_1.z.number().positive().default(100).describe('Maximum search radius in km'),
    grade: core_1.z.enum(['A', 'B', 'C']).optional().describe('Minimum grade requirement'),
    limit: core_1.z.number().int().min(1).max(50).default(20),
});
const LocationRecommendationSchema = core_1.z.object({
    ...
    max_radius_km: core_1.z.number().positive().default(100),
    max_price_per_kg: core_1.z.number().positive().optional(),
});
const GetContactSchema = core_1.z.object({
    listing_id: core_1.z.string().uuid().describe('Listing ID to get seller contact for'),
});
const CompareListingsSchema = core_1.z.object({
    listing_ids: core_1.z.array(core_1.z.string().uuid()).min(2).max(5).describe('Up to 5 listing IDs to compare side-by-side'),
});
class SourcingTools {
    async searchMaterials(input, ctx) {
        ...
        if (input.buyer_lat && input.buyer_lng && listings.length > 0) {
            listings.sort((a, b) => {
                const distA = haversineDistance(input.buyer_lat, input.buyer_lng, a.location?.lat || 0, a.location?.lng || 0);
                const distB = haversineDistance(input.buyer_lat, input.buyer_lng, b.location?.lat || 0, b.location?.lng || 0);
                return distA - distB;
            });
        }
        return {
    ...
    async recommendBestPlaceToSource(input, ctx) {
        const supabase = (0, supabase_service_js_1.getSupabaseClient)();
        ...
        for (const [zoneName, center] of Object.entries(zoneMap)) {
            ...
            if (distToBuyer > input.max_radius_km)
                continue;
            const zoneSellers = [];
            ...
            if (zoneSellers.length > 0) {
                const avgPrice = zoneSellers.reduce((sum, s) => sum + s.seller_quoted_price_per_kg, 0) / zoneSellers.length;
                const avgTrust = zoneSellers.reduce((sum, s) => sum + (s.factories?.trust_score || 0), 0) / zoneSellers.length;
                zoneStats.push({
                    name: zoneName,
                    ...
                    top_listing_ids: zoneSellers.slice(0, 3).map((s) => s.id),
    ...
    async getSellerContact(input, ctx) {
        const supabase = (0, supabase_service_js_1.getSupabaseClient)();
        ...
        if (!factory)
            throw new Error('Seller factory not found');
    ...
    async compareListings(input, ctx) {
        const supabase = (0, supabase_service_js_1.getSupabaseClient)();
        const { data: listings } = await supabase
            .from('listings')
        ...
        if (!listings || listings.length === 0)
            throw new Error('No listings found for comparison');
...
__decorate([
    ...
    (0, core_1.UseGuards)(),
    (0, core_1.Cache)({ ttl: 300, key: (input) => `search:${JSON.stringify(input)}` }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [void 0, Object]),
    __metadata("design:returntype", Promise)
], SourcingTools.prototype, "searchMaterials", null);
__decorate([
    (0, core_1.ToolDecorator)({
    ...
    (0, core_1.Cache)({ ttl: 1800 }),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [void 0, Object]),
    __metadata("design:returntype", Promise)
], SourcingTools.prototype, "recommendBestPlaceToSource", null);
__decorate([
    (0, core_1.ToolDecorator)({
    ...
    __metadata("design:paramtypes", [void 0, Object]),
    __metadata("design:returntype", Promise)
], SourcingTools.prototype, "getSellerContact", null);
__decorate([
    (0, core_1.ToolDecorator)({
    ...
    __metadata("design:paramtypes", [void 0, Object]),
    __metadata("design:returntype", Promise)
], SourcingTools.prototype, "compareListings", null);
...
function predefinedIndustrialZones() {
    return {
...
//# sourceMappingURL=sourcing.tools.js.map